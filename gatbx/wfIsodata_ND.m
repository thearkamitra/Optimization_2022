...
% 	end;
	...
	if next==1,
        ...
        [Di, D, STM]= dispersion(X, Z, cluster, A);
		...
		while (hubo_division==0) & (i < A),      % Si para algun...                                     
		    ...
		    sindex=size(index);                                                                         
		    sindex=sindex(2);                                                                           
% 		    if  (STM(i)>OS) & (  ((Di(i,1)>D(1)) & (Di(i,2)>D(2)) & (Di(i,3)>D(3)) & (sindex>(2*(ON+1)))) | (A<=(k/2)) ), 
		    if  (STM(i)>OS) & (  ((Di(i,:)>D) & (sindex>(2*(ON+1)))) | (A<=(k/2)) )
		        hubo_division=1;                                                                        
		        next=1;                                                                                 
		        [Z, cluster]=dividir(STM, A, cluster, Z, i, (A+1), X);    % Division.                 
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if next==2
        [iter,final,vuelve3]= termina_o_itera(iter, I);
    end;
...
% end;
for i=1:A
    d(:,i) = sqrt(sum((X-ones(size(X,1),1)*Z(i,:)).^2,2));
end
dmin = min(d,[],2);
elim_idx = find(dmin > min_dist);
cluster(elim_idx)=0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [m] = wfcercano(X, Z, k)
    d=zeros(size(X,1), k);
    for j=1:k
%         P=x;    %[x y x3];
%         d=distancia(Z(j,:), P); % Distancia del centro al punto.
        d(:,j) = sqrt(sum((X-ones(size(X,1),1)*Z(j,:)).^2,2));
    end
    [dmin, d_idx] = min(d,[],2); 
    m = d_idx';
    
% function [m] = cercano(x, Z, k)
%     dtemp=0;
%     d=0;
%     for j=1:k
%         P=x;    %[x y x3];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Zout1] = recalcula(cluster, X, k, Z) %Realiza la media de los puntos asigandos a cada clase corersp. a los centros
	s=size(X);
	s=s(1);
	valor=zeros(1,k);
	Zout1=zeros(k,size(X,2));
	for m=1:k
        index=find(cluster==m);
        if isempty(index)==0
            sindex=size(index);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Ditemp, Dtemp, STMAX] = dispersion(X, Z, cluster, A)
    dimN = size(X,2);
    Ditemp=zeros(A,dimN);
    Dtemp=zeros(1,dimN);
    ST=zeros(A,dimN);
...
%             suma = suma + d*X(j,:);
%         end;
        d = sqrt(sum((X(index,:)-ones(sindex(2),1)*Z(i,:)).^2,2));
        for k = 1:dimN
            suma(k) = sum(d.*X(index,k));
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Ztemp, Atemp, clustertemp]=eliminar(A, cluster, Z, X, ON)
	%Asignacion.
	...
	if  isempty(vacio)==0
%         for i=vacio,
%             clustertemp(i)=cercano(X(i,:), Ztemp, Atemp);
%         end;
        clustertemp(vacio)=wfcercano(X(vacio,:), Ztemp, Atemp);
	end;
...
%         end;
%     end;
    d_ncentr = sqrt(sum((X(dividendo,:)-ones(length(dividendo),1)*Ztemp(ncentro,:)).^2,2));
    d_Atemp = sqrt(sum((X(dividendo,:)-ones(length(dividendo),1)*Ztemp(Atemp,:)).^2,2));
    Atemp_idx = find(d_ncentr>=d_Atemp);
    clustertemp(dividendo(Atemp_idx))=Atemp;
...
%     dist=sqrt( ((Z1(1)-Z2(1))^2) + ((Z1(2)-Z2(2))^2) +((Z1(3)-Z2(3))^2));% Distancia entre dos puntos.
    dist=sqrt( sum((Z1-Z2).^2));
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Inicializa los centros  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Z]=inicializa_centros(X, k)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [itertemp,FINtemp,vuelve3temp]= termina_o_itera(iter, I)
    NO = 1;
    itertemp=iter;
    FINtemp=0;
    vuelve3temp=1;
